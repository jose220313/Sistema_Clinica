...
@permission_required('base.is_surgeon')
def CrearPaciente(request):
    if request.method == "POST":
        ...
        return redirect('VerPaciente_Cirujano')
    
    return render(request, "base/paciente_form.html")
...
def CrearCita(request):
    ...
    if request.method == "POST":
        ...
        return redirect('Cita_Cirujano')
    return render(request, "base/cita_form.html", context)
...
def ActualizarPaciente(request, id_paciente):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            form.save()
        return redirect("VerPaciente_Cirujano") 
    return render(request, "base/paciente_actualizar_form.html", {"form": form,"pac":idpaciente}) 
...
def ExpedienteView(request, id_paciente):
    ...
    template_name = 'base/expediente_form.html'
    return render(request,template_name, context)
...
def ExpedienteSignosVitales(request, id_expediente):
    ...
    template_name = 'base/expediente_view.html'
    return render(request,template_name,context)
...
def ExpedienteConsulta(request, id_expediente):
    query = Expediente.objects.filter(id_expediente=id_expediente)
    context = {'SignosVitales':query}
    template_name = 'base/expediente_consulta.html'
    return render(request,template_name,context)
...
def ExpedienteUpdate(request,id_expediente):
    ...
    return render(request,'base/expediente_update.html',{"ex":ex})


class ActualizarCita(UpdateView):
    model = Cita
    form_class = CitaForm
    template_name = "base/cita_update.html"
    success_url = reverse_lazy("Cita_Cirujano")

class BorrarCita(DeleteView):
    model = Cita
    form_class = CitaForm
    template_name = "base/cita_confirm_delete.html"
    success_url = reverse_lazy("Cita_Cirujano")

@permission_required('base.is_surgeon')
def home_surgeon(request):
